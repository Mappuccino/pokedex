...
async function CatchThemAll(){
    ...
    try {
        for(var i = 1; i <= pokemonAll; i++) {
            var risposta = await fetch(URL + i)
                ...
                if (!risposta.ok) {
                    throw new Error("Errore durante il caricamento del Pokedex.");
                }
            }     
    }
    catch (error) {
...
async function mostraPokemon(poke) {
    ...
    else if (pokeId.length === 3 || pokeId.length === 4){
        return pokeId;
